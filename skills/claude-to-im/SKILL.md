...
- All normal QQ replies use the same agent path; simple conversation can finish in one model round, while missing context, web facts, memory, or management actions can enter the existing internal tool loop. The main model judges from full context whether the turn is casual chat or a substantive deliverable. Problem solving/proofs/calculations, code, writing/translation, summaries and short continuations such as “第19题/继续/过程呢” must execute and deliver the actual result instead of returning an acknowledgement or placeholder. Their visible length is completion-driven rather than capped by the casual-chat budget.
...
- Social conversation memory is persisted separately at `/root/Codex-Remote-Contact/data/qq-conversation-memory.json`. Version 4 gives every group/person/private impression a brief and detailed description plus optional unified-person promotion metadata. QQ id is the stable person key; group cards, group nicknames, private-chat names and Bot-managed aliases merge under that key. Direct sender/reply/@ and known QQ-number matches take precedence; a textual alias is accepted only when it uniquely identifies one stored person. Group impressions stay scoped to their group. The main model may emit `scopeImpressionSummary/Detail`, `personImpressionSummary/Detail`, `personImpressionComplete`, `personImpressionPromotionReason`, and `botThoughtSummary/Detail` inside an invisible `[[qq_memory:{...}]]` patch. A complete flag is an AI maturity judgment and is accepted only when the existing stable, non-sensitive profile has a substantive brief and detail. Legacy fields are still accepted, a newer profile overwrites the same stable id, and `/新对话` keeps these impressions.
