...
| `src/qq-task-control.js` | Parse and bound optional progress, continuation, extra-minute and extra-round requests for multi-step QQ tasks. |
...
- All normal QQ replies use the same agent path; simple conversation can finish in one model round, while any internal capability that materially improves correctness, context continuity, memory quality, evidence, or real execution should be used proactively instead of being reserved for failure. The main model judges from full context whether the turn is casual chat or a substantive deliverable. Problem solving/proofs/calculations, code, writing/translation, summaries and short continuations such as “第19题/继续/过程呢” must execute and deliver the actual result instead of returning an acknowledgement or placeholder. Their visible length is completion-driven rather than capped by the casual-chat budget. During a complex task the model may optionally send a few substantive progress updates at any useful stage, not only before work starts, and may also stay silent until the result.
...
- Social conversation memory is persisted separately at `/root/Codex-Remote-Contact/data/qq-conversation-memory.json`. Version 4 gives every group/person/private impression a brief and detailed description plus optional unified-person promotion metadata. QQ id is the stable person key; group cards, group nicknames, private-chat names and Bot-managed aliases merge under that key. Direct sender/reply/@ and known QQ-number matches take precedence; a textual alias is accepted only when it uniquely identifies one stored person. Group impressions stay scoped to their group. The main model may emit `scopeImpressionSummary/Detail`, `personImpressionSummary/Detail`, `personImpressionComplete`, `personImpressionMemorable`, `personImpressionPromotionReason`, and `botThoughtSummary/Detail` inside an invisible `[[qq_memory:{...}]]` patch. `complete` is an AI maturity judgment; `memorable` is reserved for an unusually salient interaction that creates likely-persistent Bot interest or impression before the full profile is mature. Either can request unified-person promotion, but only when the existing stable, non-sensitive profile has a substantive brief and detail. Salient people/events should also trigger proactive evaluation of current-scope `/记忆` search/overwrite/add rather than waiting for an explicit “remember this.” Legacy fields are still accepted, a newer profile overwrites the same stable id, and `/新对话` keeps these impressions.
...
- The QQ bot itself can use internal tools that are not shown in `/菜单`: it may emit `[[qq_command:/...]]` for permitted menu actions, chat history, current-scope memory, unified/person memory, long-term knowledge, and web lookup. Internal tools are not a last resort; use them whenever they materially improve the result. The Hub supports an agent-style multi-round loop and strips `[[qq_done]]` before sending. `[[qq_progress:...]]` optionally sends one substantive progress bubble at any model/tool stage; `[[qq_task_continue]]` requests another model round without an external tool; `[[qq_task_budget:{"extraMinutes":5,"extraRounds":4,"reason":"..."}]]` requests bounded extra per-turn time and loop rounds. Hard limits are 4 progress bubbles, 2 budget requests, 15 extra minutes and 8 extra rounds per request, and 24 total loop rounds. A progress message is receipt-bearing and enters recent Bot context only when delivered. Internal menu commands always use the original sender's permissions; they must never grant owner-equivalent authority. Any visible claim that a QQ write action completed—including poke, friend/group add, request handling, moderation, or QQ Space writes—must follow the corresponding tool call and an explicit successful result. The model may refuse before execution, but must report an already successful write and must never claim success after failure.
