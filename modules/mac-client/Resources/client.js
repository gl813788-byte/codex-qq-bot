...
function renderQqSelfPersona(summary) {
  ...
  $("#qqSelfPersona").innerHTML = `<article class="behavior-frequency-item persona-summary-card"><div class="persona-summary-head"><strong>${escapeHtml(name)}</strong><small>${escapeHtml(revision)}</small></div><p>${escapeHtml(progress)}</p>${description ? `<p class="persona-summary-copy">${escapeHtml(description)}</p>` : ""}<section><h4>${escapeHtml(t("selfPersonaKeywords"))}</h4><div>${keywordChips}</div></section><section><h4>${escapeHtml(t("selfPersonaTopics"))}</h4><div>${interestChips}</div></section></article>`;
}
