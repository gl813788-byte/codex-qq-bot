...
A confirmed delivered Bot bubble clears the group's pre-delivery ordinary-interest pending cycle, so cadence and minute timing restart only from later human messages. An in-flight old judge is superseded without consuming those later messages.
