...
这个项目同时涉及 Node.js、Codex 登录、QQ/OneBot、配置文件、后台进程和端口检查。仓库已提供会保留已有配置的中文 `一键部署.command`，但不同系统的路径、OneBot 实现和现有运行方式仍可能不同。让 Codex 作为部署操作员，可以先读机器现状，再选择最小、安全且可验证的动作。
...
`一键部署.command` 是中文启动器，首次部署状态机在仓库 `ncc` 内。`ncc` 首次会调用底层部署准备，检查工具、创建缺失的本地文件、安装 npm 依赖并运行 `npm run verify`；发现无关全局 `ncc` 时会明确跳过覆盖。成功后 `ncc` 切换为常规控制菜单。
