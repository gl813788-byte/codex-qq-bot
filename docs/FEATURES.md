...
When one fused answer contains Bot-triggering messages from multiple people, the main model receives the exact participant list. Every listed person is selectable for either a QQ quote with hidden `[[qq_reply:quote:QQ-number]]` or a real sender mention with `[[qq_reply:mention:QQ-number]]`; `[[qq_reply:plain]]` selects neither. If the model emits no marker, the Hub also uses plain delivery, so an answer to a later participant cannot accidentally quote or mention the first trigger. The single-sender path retains the existing relationship-based quote/mention/plain choice, and visible `@exact-name ` / `@QQ-number ` remains available for explicit or multiple mentions.
...
Owner capabilities include status/configuration, model and reasoning selection, proactive policy, group allowlists, bans, command permissions, moderation, request handling and notifications. When the local NapCat social extension supports them, owner-only tools can also initiate friend/group requests and perform QQ Space reads or writes. Active friend-add tools accept the canonical `/主动加好友` command plus the `/加好友` and `/添加好友` aliases. Stranger UID lookup is optional: failure does not block submission, and loopback-only `/inspect-friend` diagnoses a target without sending. On QQ builds that expose the modern `AddBuddyService`, the bridge uses it for both verification preflight and submission; this avoids the stale NapCat `reqToAddFriends` declaration and reports a missing verification message before any request is sent. Older BuddyService and group wrappers retain their guarded request-object/two-argument adapters, and only an explicit native argument-count assertion permits a compatibility retry, so other failures cannot duplicate a request. A `submitted` result does not mean the peer accepted the request. QQ Space publishing supports `/发动态 text`, `/发动态 图片=当前`, and `/发动态 text | 图片=当前`, with up to nine current/quoted images copied into the task workspace; arbitrary server paths are rejected. Comment requests use the current topic-ID and charset fields.

Non-owner menu visibility and execution come from the same permission keys. Owner IDs and the Bot itself cannot be demoted, banned, muted or kicked by delegated users. Separately, the main-model system prompt fixes QQ `3784642920` as this project's developer; that project identity is not derived from, and does not change with, the current owner list. Owner authority still comes only from the Hub's trusted `isOwner` decision.
