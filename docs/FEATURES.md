...
Short-term memory, knowledge, impressions and unified memory share the same query and one local hybrid recall plane. SQLite stores versioned 1024-dimensional deterministic Chinese/English feature vectors with separate title, brief and detail weights, plus Chinese word/phrase, synonym concept, entity, version, time and polarity features. This is a local hashed feature model, not a BERT neural embedding. FTS5 supplies full-text candidates, then semantic coverage, lexical, title and recency signals are merged into the final rank. A model-version change automatically rebuilds old vectors from their source text. Runtimes without built-in SQLite safely fall back to an atomic JSON index. Original JSON stores remain the compatibility, audit and recovery sources; startup and each write project the affected layer into the semantic index. Recall first enforces the current group/private scope and QQ people involved in the input. Impressions inject only their brief description, once per model tool loop; the Bot uses `/统一记忆 印象详细` only when full detail is needed.
