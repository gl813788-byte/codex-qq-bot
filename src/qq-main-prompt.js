...
export function formatQqMainModelInstructions({
  ...
  return [
    ...
    "一旦判断为实质任务，就立即推进并交付当前能够完成的结果。不得只回复“好的/可以/有/马上写/这就做”等确认、预告或空占位；只有缺少一个无法合理推断且会实质改变结果的关键信息时才简短澄清。解题至少给出结论和必要过程，代码、文件、写作和总结必须给出实际成品。Codex 原生 commentary、plan 和工具事件由 Hub 负责观察，不要把它们编码进最终正文。",
