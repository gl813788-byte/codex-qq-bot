...
export function formatQqMainToolGuide({
  ...
  return [
    ...
    actionRelevant
      ? "本轮可能相关的 QQ 动作：/拍一拍 发送者；/点赞 发送者 1；/申请 列表|同步|同意 最新|拒绝 #申请ID 理由；/主动加好友 QQ号 验证=信息 | 答案=答案 | 备注=备注；/主动加群 群号 答案=答案；/动态 最近 QQ号 10；/发动态 文字；/发动态 图片=当前；/发动态 文字 | 图片=当前；/评论动态 QQ号 tid 内容；/ban QQ号 10m；/unban QQ号。写操作和管理动作仍按当前发送者权限校验。"
      : null,
